 ...
 * @param {Number} exhibition_url
 * @param {Array} work_idx
 */
Board.prototype.loadWorks = function (length, exhibition_url, work_idx) {
	for (var i = 0; i < length; i++) {
		$('<div class="work">' +
				'<div class="inner">' +
					'<div class="workLabel">' +
					'</div>' +
					'<img class="workImg" src=' + '"/temp/' + exhibition_url + '_' + work_idx[i] + '.png" />' +
	...
	for (var i = 0; i < length; i++) {
		$('<div class="work_thumb">' +
				'<div class="inner">' +
					'<img src=' + '"/temp/' + exhibition_url + '_' + work_idx[i] + '.png" />' +
				'</div>' +
...
Board.prototype.moveByClickWork = function (e) {
	// 메인 그림 눌렀을 때 움직이지 않음
	if (e.currentTarget.parentNode.id) return;
...
Board.prototype.init = function () {
	// 작품 서버에서 로드
	this.loadWorks(length, exhibition_url, work_idx);
	// Board객체 프로퍼티에 작품 등록
